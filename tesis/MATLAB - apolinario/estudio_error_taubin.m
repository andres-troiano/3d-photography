clear variables
path = 'C:\Users\60069978\Documents\MATLAB\medicion03\';
set(0,'DefaultFigureVisible', 'off');

% las unidades son mm
iteraciones = 1000;
    
% error = [0.02 0.15 0.5 0.7];
% []

error_pos = [0.02 0.15 0.5 0.7];
...
for k = 1:numel(error_pos)
    
    for j = 1:numel(angulo_barrido)

        % el error está entre 20 y 700 um
        % error = 0.02;
        % esto está en grados
        ...
        for i = 1:iteraciones
            ...
            %%%%%%%%%% usando distribución uniforme %%%%%%%%%%
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            
%             % acá no estoy asignando el error como
%             % radial, sino que se lo sumo a x y a y.
%     
%             % sin embargo de esta manera se generan algunos artificios que en la
%             % vida real no pasan, como que la curva de datos vuelva sobre sí misma
%             dist_error = error_pos(k)*rand(1, N);
%             x = r*cos(t) + dist_error;
%             
%             dist_error = error_pos(k)*rand(1, N);
%             y = r*sin(t) + dist_error;
...
%         saveas(h1, [path tag1], 'png');
%         saveas(h1, [path tag1]);
% 
%         saveas(h2, [path tag2], 'png');
%         saveas(h2, [path tag2]);
%         
%         saveas(h4, [path tag4], 'png');
%         saveas(h4, [path tag4]);
%         
%         saveas(h5, [path tag5], 'png');
%         saveas(h5, [path tag5]);
...
grid on
xlabel('Ángulo (º)')
ylabel('Diámetro ajuste menos real (\mum)')
