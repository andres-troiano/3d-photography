function calculateCalibration_con_fronteras(C,basepath,ind_fronteras)
...
for q=1:2
    ...
    % aplico la condici�n de pertenencia a la regi�n v�lida
...
%     saveas(f5, [basepath 'figuras_calibracion\f5_cam_' num2str(q)])
    saveas(f5, [basepath 'figuras_calibracion_fronteras/f5_cam_' num2str(q) '.png'])
...
% 	saveas(f4, [basepath 'figuras_calibracion\f4_cam_' num2str(q)])
    saveas(f4, [basepath 'figuras_calibracion_fronteras/f4_cam_' num2str(q) '.png'])
...
%     saveas(f3, [basepath 'figuras_calibracion\f3_cam_' num2str(q)])
    saveas(f3, [basepath 'figuras_calibracion_fronteras/f3_cam_' num2str(q) '.png'])
...
%     saveas(f2, [basepath 'figuras_calibracion\deltas_cam_' num2str(q)])
    saveas(f2, [basepath 'figuras_calibracion_fronteras/deltas_cam_' num2str(q) '.png'])
...
%     saveas(f1, [basepath 'figuras_calibracion\used_points_cam_' num2str(q)])
    saveas(f1, [basepath 'figuras_calibracion_fronteras/used_points_cam_' num2str(q) '.png'])

end

save(fullfile(basepath,'calibration_con_fronteras.mat'),'px2mmPol')
