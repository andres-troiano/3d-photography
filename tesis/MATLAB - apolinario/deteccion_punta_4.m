...
plot(punta_x(indice_2), punta_y(indice_2), '*r')
plot(punta_x, [recta_decreciente; recta_decreciente + 3*delta_decreciente; recta_decreciente - 3*delta_decreciente], 'k--')
plot(punta_x, [recta_creciente; recta_creciente + 3*delta_creciente; recta_creciente - 3*delta_creciente], 'k--')
...
% ESTO ESTÁ MAL. Al tirar cosas el indice se pasa del mínimo y empieza a
% evaluar cosas de la otra cara

% Para evitar esto, hago la limpieza creando vectores nuevos:
punta_x_tmp = [];
punta_y_tmp = [];
...
for i = 1:indice_2(1)
    ...
    if (punta_y(i) > recta_decreciente(i) - 3*delta_decreciente(i)) || (punta_y(i) < recta_decreciente(i) + 3*delta_decreciente(i))
        punta_x_tmp = [punta_y_tmp punta_x(i)];
        punta_y_tmp = [punta_y_tmp punta_y(i)];
    end
end

% ahora como tiré cosas tengo que recalcular indice 2
quiebre_y = min(punta_y); 
indice_2 = find(punta_y == quiebre_y);
...
% y vuelvo a ajustar
[p_decreciente, S_decreciente] = polyfit(punta_x(1:indice_2), punta_y(1:indice_2), 1);
[recta_decreciente, delta_decreciente] = polyval(p_decreciente, punta_x, S_decreciente);

[p_creciente, S_creciente] = polyfit(punta_x(indice_2:end), punta_y(indice_2:end), 1);
[recta_creciente, delta_creciente] = polyval(p_creciente, punta_x, S_creciente);
...
% plot(punta_x(indice_2), punta_y(indice_2), '*g')
% plot(indice_3, perfil(indice_3), '*c')
% plot(punta_x, punta_y, '.-r')
plot(punta_x, [recta_decreciente; recta_decreciente + delta_decreciente; recta_decreciente - delta_decreciente], 'k--')
plot(punta_x, [recta_creciente; recta_creciente + delta_creciente; recta_creciente - delta_creciente], 'k--')
plot(x_0, y_0, 'r.')