function [centro_x, centro_y, r_teorico, r_individual, centro_individual] = mido_patron_centros_coincidentes_base(frames_cilindro, id_cilindro, px2mmPol, F, FC, path_plot)

    % modificaci�n de "mido_patron", para correr una de las c�maras de
    % manera que los centros de las circunferencias medidas por las 2
    % c�maras individualmente coincidan
    ...
    for q = 1:2
        ...
        % esto lo comento porque ya que voy a hacer coincidir los centros,
        % no quiero hacer ningún uso del offset
...
% 
        ...
        % grafico los ajustes individuales para monitorear %
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         x_teorico = r_individual(q) * cos(t) + centro_x;
%         y_teorico = r_individual(q) * sin(t) + centro_y;
%         
%         plot(centro_x, centro_y, '+')
...
%         xlabel('X (mm)')
%         ylabel('Y (mm)')
%         tit = sprintf('%s - C%d', id_cilindro, q);
%         title(tit)
    ...
    % ojo, esto no se puede hacer despu�s de concatenar los 2 perfiles
    
    % acá falta correr F y FC (F viene corrida de fábrica, pero yo la
    % des-corrí)
    % la voy a correr con el offset que estamos calculando acá al hacer
    % coincidir los centros:
    F = {[F{1}(:,1), F{1}(:,2)], [F{2}(:,1) - offset_centros(1), F{2}(:,2) - offset_centros(2)]};
    FC = {[FC{1}(:,1), FC{1}(:,2)], [FC{2}(:,1) - offset_centros(1), FC{2}(:,2) - offset_centros(2)]};
    
    plot(XY{1}(:,1),XY{1}(:,2),mc{1})
    plot(F{1}(:,1), F{1}(:,2), mf{1})
    plot(FC{1}(:,1), FC{1}(:,2), mfc{1})
    plot(XY{2}(:,1),XY{2}(:,2),mc{2})
    plot(F{2}(:,1), F{2}(:,2), mf{2})
    plot(FC{2}(:,1), FC{2}(:,2), mfc{2})
    
    % las fronteras no me molesto en correrlas porque de ac� en m�s no las
    % uso m�s
    ...
    xlabel('X (mm)')
    ylabel('Error radial (mm)')
    tit = sprintf('Residuos 2 cámaras - %s', id_cilindro);
    title(tit)
