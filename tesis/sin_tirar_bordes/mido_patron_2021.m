function [centro_x, centro_y, r_teorico, r_individual, centro_individual] = mido_patron_2021(frames_cilindro, id_cilindro, px2mmPol, offset, F, FC, path_plot)
    ...
    for q = 1:2
        ...
        py = median(frame);
        px = 1:size(py,2);
        [px, py] = tiro_datos_nulos_perfil_2021(px, py);
        px = px.';
        py = py.';

        py = 1088-py;

        x = polyval4XY_2021(px2mmPol{q}(1), px, py);
        y = polyval4XY_2021(px2mmPol{q}(2), px, py);
        
        % convierto a mm las fronteras dadas por la calibraci�n
        temp_x = polyval4XY_2021(px2mmPol{q}(1), FC{q}(:,1), FC{q}(:,2));
        temp_y = polyval4XY_2021(px2mmPol{q}(2), FC{q}(:,1), FC{q}(:,2));
        FC{q} = [temp_x, temp_y];
...
%         saveas(h2, [path_plot id_cilindro '_error_C' num2str(q) '.png'])
        ...
        angulo = calculo_angulo_2021(P1, P0, P2, P0);
