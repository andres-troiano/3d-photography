function [] = calculateIntersections_hexagono_curvo(basepath)
    ...
    for q=1:2
        ...
        km=find(X==Xm & Y==Ym);
        % Monstrar al usuario la figure para que elija la posici�n aproximada de la intersecci�n. 
        nfig=figure();plot(Profiles(:,km), '.-'),hold all,grid on,xlabel('Pixel X'),ylabel('Pixel Y'),axis equal,title({sprintf('x=%d, y=%d',X(km),Y(km)),'Elija la intersecci�n aproximada o cierre la ventana'})
...
%             p=ginput(1)
        ...
        %%
        clc
        C{q}=nan(size(X,1),9); %xc,yc,X,Y,k,estd1,n1,estd2,n2,x1,x2,x3,x4,y1,y2,y3,y4
        C_curvo{q}=nan(size(X,1),9);
        xguess=round(p(1));
        o=0;
        show=false;
        if show==true
        ...
        for k=ind'
            ...
            % agrego el ajuste por parábolas
            [P1,estd1_curvo,n1_curvo]=fitCurvedLine(x,y,ind1);
    ...
    %			figure,plot(Profiles(:,k),'.-'),title(sprintf('o=%d, error en L1 ',o))
            ...
            % agrego el ajuste por parábolas
            [P2,estd2_curvo,n2_curvo]=fitCurvedLine(x,y,ind1);
    ...
    % 		if P(3)==0
            ...
            C_curvo{q}(k,:)=[xc_parabolas yc_parabolas X(k) Y(k) k estd1_curvo n1_curvo estd2_curvo n2_curvo]; % C_curvo
...
%     saveas(f3, [basepath 'figuras_intersecciones\f3_cam_'])
...
%     saveas(f4, [basepath 'figuras_intersecciones\f4_cam_' ])
...
%     saveas(f5, [basepath 'figuras_intersecciones\f5_cam_'])
...
%     saveas(f6, [basepath 'figuras_intersecciones\f6_cam_'])
