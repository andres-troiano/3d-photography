...
% calcular el offset, es decir calcula el +-35 individual de cada cámara,
% con esa restricción calcula el offset y después se hace el ajuste
% global.
...
plot(error(:,1), error(:,2), '.b')
plot(F{1}(:,1), F{1}(:,2), '--b')
plot(F{2}(:,1), F{2}(:,2), '--r')
title('Puntos válidos para calcular el offset')
