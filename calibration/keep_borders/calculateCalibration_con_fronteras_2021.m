function calculateCalibration_con_fronteras_2021(C,basepath,ind_fronteras)
...
% 	ind1=~ind1 & ~ind2 & ~ind3 & ~ind4 & ~isnan(C{q}(:,1));
    ...
    % aplico la condici�n de pertenencia a la regi�n v�lida (esto por ahora
    % lo dejo)
    ind1 = ind1 & ind_fronteras{q};
    
	% ind1=~isnan(C{1}(:,1));
	Xmm=C{q}(ind1,3);
	Ymm=C{q}(ind1,4);
	Xpx=C{q}(ind1,1);
	mm2pxPol{q}(1)=polyfit4XY_2021(Xmm,Ymm,Xpx);
	Xpxfit=polyval4XY_2021(mm2pxPol{q}(1),Xmm,Ymm);
	% figure,plot3(X,Y,Xfit,'.'),hold all,plot3(X,Y,Xraw,'.')
%     saveas(f5, [basepath 'figuras_calibracion/f5_cam_' num2str(q)])
    saveas(f5, [basepath 'figuras_calibracion_con_fronteras/f5_cam_' num2str(q) '.png'])
	...
	Ypx=C{q}(ind1,2);
	mm2pxPol{q}(2)=polyfit4XY_2021(Xmm,Ymm,Ypx);
	Ypxfit=polyval4XY_2021(mm2pxPol{q}(2),Xmm,Ymm);
...
% 	saveas(f4, [basepath 'figuras_calibracion/f4_cam_' num2str(q)])
    saveas(f4, [basepath 'figuras_calibracion_con_fronteras/f4_cam_' num2str(q) '.png'])

	%% Invertir la calibraci�n para poder calcular x,y reales a partir de pixels.
	ind=mm2pxPol{q}(1).ind & mm2pxPol{q}(2).ind;
	Xpxfit1=Xpxfit(ind);
	Ypxfit1=Ypxfit(ind);
	px2mmPol{q}(1)=polyfit4XY_2021(Xpxfit1,Ypxfit1,Xmm(ind));
%	Xmmfit=polyval4XY_2021(px2mmPol{q}(1),Xpxfit1,Ypxfit1);
	Xmmfit=polyval4XY_2021(px2mmPol{q}(1),Xpx(ind),Ypx(ind)); % Error total de la calibraci�n, comparando con los valores medidos, no los utilizados en el ajuste de "inversi�n"
	Xr=Xmm(ind);
	...
	px2mmPol{q}(2)=polyfit4XY_2021(Xpxfit1,Ypxfit1,Ymm(ind));
%	Ymmfit=polyval4XY_2021(px2mmPol{q}(2),Xpxfit1,Ypxfit1);
	Ymmfit=polyval4XY_2021(px2mmPol{q}(2),Xpx(ind),Ypx(ind)); % Error total de la calibraci�n, comparando con los valores medidos, no los utilizados en el ajuste de "inversi�n"
	Yr=Ymm(ind);
...
%     saveas(f3, [basepath 'figuras_calibracion/f3_cam_' num2str(q)])
    saveas(f3, [basepath 'figuras_calibracion_con_fronteras/f3_cam_' num2str(q) '.png'])
...
%     saveas(f2, [basepath 'figuras_calibracion/deltas_cam_' num2str(q)])
    saveas(f2, [basepath 'figuras_calibracion_con_fronteras/deltas_cam_' num2str(q) '.png'])
...
%     saveas(f1, [basepath 'figuras_calibracion/used_points_cam_' num2str(q)])
    saveas(f1, [basepath 'figuras_calibracion_con_fronteras/used_points_cam_' num2str(q) '.png'])

end

save(fullfile(basepath,'calibration_con_fronteras_2021.mat'),'px2mmPol')
